...
public:
  ...
  //Min: A constructor for replay by sharing solver
  Executor(const InterpreterOptions &opts, InterpreterHandler *ie, TimingSolver* solver, std::vector<int> trace);
  ...
  //Min PathSkipper Add-on 
  public:
  bool flagCopying;
  bool termOnError;
  ExecutionState* loopState;
  ExecutionState* backUpState;

  ExecutionState& checkJump(ExecutionState& current);
  int replayCase(ExecutionState& current);
  int replaySuccess(ExecutionState& current, KTest* out);
  void copyStateStackStat(ExecutionState& backUp, ExecutionState& current);
  void restoreLastState(ExecutionState& backUp, ExecutionState& current);
  void removeLargestQueryHist();
  void togglePshEnable();
  void togglePshInReplay();

  std::vector<int> replayTrace;
  unsigned replayTraceIdx;
  std::unordered_map<ExecutionState*, int> copyHistMap;
  int copyHistLimit;
  std::unordered_map<llvm::BasicBlock*, llvm::BasicBlock*> exitHeadMap;
  std::unordered_map<llvm::BasicBlock*, std::unordered_set<llvm::BasicBlock*>> headExitMap;
  std::unordered_map<const llvm::BasicBlock*, const llvm::BasicBlock*> exitEdgeMap;
  bool pshEnable;
  int prevBranchCov;
  int sameBranchCnt;

  std::queue<ExecutionState*> priorityStates;
  //T: loops
  std::vector<std::vector<llvm::BasicBlock*>> loop_container;
